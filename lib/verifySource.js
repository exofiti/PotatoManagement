...
function getVerifyDb(env = process.env) {
    ...
    if (!require('fs').existsSync(resolved)) {
        console.warn(`[verifySource] SQLite 파일을 찾을 수 없습니다: ${resolved}`);
        return null;
    }
    ...
    try {
        ...
        try {
            ...
            console.error(`[verifySource] 데이터베이스 열기 실패 (${resolved}):`, dbError.message);
            return null;
        }
    }
}
...
function lookupSqliteByDiscordId(discordId, env = process.env) {
    ...
    try {
        ...
        console.error('[verifySource] SQLite 조회 오류:', error.message);
        return null;
    }
}
